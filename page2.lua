...
if isNoclipActivated then
    ...
    local function makePartsNoclip(part)
        ...
        part.Visible = false -- Make parts invisible
    end
    ...
    local function makePartsClip(part)
        ...
        part.Transparency = 0
        part.Visible = true -- Make parts visible
    end
...
-- Universal player hitbox expander
local function expandHitbox(player)
    ...
    local function onPartAdded(part)
        if part:IsA("BasePart") then
            part.Size = part.Size + Vector3.new(2, 2, 2) -- Expand hitbox
        end
    end
    ...
    character.ChildAdded:Connect(onPartAdded)
end
...
game.Players.PlayerAdded:Connect(expandHitbox)
